...
type Props = {
  initialCasse: Cassa[]
  initialMovimenti: Movimento[]
  luoghi: Luogo[]
  soci: Socio[]
  initialPosizioniAperte: PosizioneSocio[]
  initialDebitiClienti: any[]
  initialDebitiFornitori: any[]
  riferimenti: Riferimento[]
  cassaUnica: { id: number; nome: string } | null
}

export function ContabilitaClient({
  initialCasse, initialMovimenti, luoghi, soci,
  initialPosizioniAperte, initialDebitiClienti, initialDebitiFornitori,
  riferimenti, cassaUnica,
}: Props) {
  const [casse, setCasse] = useState(initialCasse)
  const [movimenti, setMovimenti] = useState(initialMovimenti)
  const [posizioniAperte] = useState(initialPosizioniAperte)
  const [debitiClienti] = useState(initialDebitiClienti)
  const [debitiFornitori] = useState(initialDebitiFornitori)
  ...
  function calcSaldoPerLuogo(luogoId: number) {
  ...
  function getTipoBadgeColor(tipo: string): 'default' | 'success' | 'warning' | 'danger' | 'info' {
  ...
  async function refreshMovimenti() {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <Card><CardContent className="p-4">
          ...
          <p className="text-2xl font-bold text-blue-700">{formatEuro(debitiClienti.reduce((a: number, d: any) => a + Number(d.importo || 0), 0))}</p>
        </CardContent></Card>
        <Card><CardContent className="p-4">
          <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-2xl font-bold text-orange-700">{formatEuro(debitiFornitori.reduce((a: number, d: any) => a + Number(d.importo || 0), 0))}</p>
        </CardContent></Card>
      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {tipiCorrenti.map(([k, v]) => {
                const Icon = v.icon; const isSelected = form.tipoMovimento === k
                return (
                  <button key={k} type="button" onClick={() => { const st = form.tipo === 'entrata' && (k === 'incasso_cliente' || k === 'rimborso_fornitore') ? 'da_riscuotere' : form.tipo === 'uscita' && (k === 'spesa' || k === 'fornitore' || k === 'acquisto') ? 'da_pagare' : form.tipo === 'entrata' ? 'riscosso' : 'pagato'; setForm({ ...form, tipoMovimento: k, stato: st }) }}
