...
export function VenditeClient({ initialData }: { initialData: Vendita[] }) {
  const [data, setData] = useState<Vendita[]>(initialData)
  ...
  async function handleEditSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const updated: Vendita = await res.json()
      setData(prev => prev.map(v => v.id === updated.id ? { ...v, ...updated } : v))
